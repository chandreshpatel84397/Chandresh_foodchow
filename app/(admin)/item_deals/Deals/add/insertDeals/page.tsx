...
export default function CreateDealPage() {
  ...
  const dealTypeId = searchParams.get("type");
  ...
  const [taxType, setTaxType] = useState<0 | 1>(0);
  ...
  useEffect(() => {
    ...
    // ✅ FIX 1: handle data.data || data wrapper
    fetch("https://localhost:44376/api/FoodShopTax/getall")
      .then((res) => res.json())
      .then((data) => setTaxes(data.data || data))
      .catch((err) => console.log(err));
  }, []);

  // ── Recalculate tax whenever price or tax changes ──────────────────
  useEffect(() => {
  ...
  const nextStep = () => {
    if (step === 1) {
      if (!formData.dealName || !formData.dealDesc) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("https://localhost:44376/api/FoodDeal/create", {
        method: "POST",
      ...
      if (res.ok) {
        Swal.fire("Success", "Deal Created Successfully", "success").then(() =>
    ...
    <div className="outer-wrapper">
      <div className="main-card">
        ...
        <p className="selected-type">
          <strong>Selected Deal Type</strong><br />{dealTypeId}
        </p>
          ...
          <div className="section-card">
            ...
            <div className="s2-white-card" style={{ marginTop: "16px" }}>
              ...
              <div className="s2-field-row">
                ...
                />
                <button className="btn-select-tax" onClick={() => setTaxPopupOpen(true)}>
                  SELECT TAX
                </button>
              </div>
              ...
              <div className="popup-overlay" onClick={() => setTaxPopupOpen(false)}>
                <div className="popup-box" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="popup-body">
                    ...
                    {/* ✅ FIX 2: Number(t.tax_type) to handle string from DB */}
                    {taxes.filter((t) => t.is_active && Number(t.tax_type) === taxType).length === 0 ? (
                      <p style={{ color: "#888", textAlign: "center" }}>No taxes available for this type</p>
                    ) : (
                      taxes.filter((t) => t.is_active && Number(t.tax_type) === taxType).map((tax) => {
                        const isChecked = selectedTaxes.some(
                          (s) => s.food_shop_tax_id === tax.food_shop_tax_id
                        );
                        const toggleTax = () => {
        ...
        {step === 4 && (
          <div className="section-card">
            ...
            <p>⚠ A Final Step !! You are about to Add Your New Deal.</p>
            <div className="btn-row">
              <button className="btn-next" onClick={handleSubmit}>ADD DEAL</button>
              <button className="btn-cancel" onClick={() => router.push("/item_deals/Deals/display")}>CANCEL</button>
            </div>
          </div>
        )}
